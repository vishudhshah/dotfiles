...
Personal dotfiles managed with [GNU Stow](https://www.gnu.org/software/stow/). All files live here and are symlinked to `$HOME` via `stow`. Editing a file anywhere (in `~/dotfiles` or the symlinked path) edits the same underlying file.
...
The script installs Homebrew if missing, runs `stow` to symlink all configs, runs `brew bundle`, and upgrades yazi plugins. After running, restart the shell with `exec zsh`.
